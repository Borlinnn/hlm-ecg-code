#!/usr/bin/env python3
"""Run deterministic k-visible-lead degradation evaluation from existing checkpoints."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate existing HLM-ECG checkpoints on deterministic k-visible leads.")
    parser.add_argument("--methods", nargs="+", default=list(DEFAULT_METHODS))
    parser.add_argument("--fill-mode", default="mean_fill", choices=["mean_fill", "zero_fill"])
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/week5_bibm_stabilization/k_visible_curve"))
...
def main() -> None:
    ...
    patterns = k_visible_random_patterns(seed=args.pattern_seed)
    selected_patterns = list(K_VISIBLE_PATTERN_ORDER)
    ...
    metadata["analysis"] = "k_visible_curve"
    write_json(output_dir / "k_visible_patterns.json", metadata)
    ...
    for run in runs:
        ...
        enriched = add_method_run_fields(result["rows"], run)
        for row in enriched:
            row["visible_lead_count"] = int(patterns[row["pattern"]].visible_count)
    ...
    write_csv(output_dir / "k_visible_metrics.csv", all_rows, metric_columns)
    write_json(
        output_dir / "k_visible_metrics.json",
        {
            **metadata,
            "rows": all_rows,
    ...
    write_markdown_table(output_dir / "k_visible_metrics.md", all_rows, metric_columns[:11])
    ...
    write_csv(output_dir / "k_visible_delta_vs_A1.csv", deltas, delta_columns)
    write_markdown_table(output_dir / "k_visible_delta_vs_A1.md", deltas, delta_columns)

    summary = summarize_multiseed(all_rows, group_cols=["method_id", "pattern", "visible_lead_count"])
    write_csv(output_dir / "k_visible_multiseed_summary.csv", summary)
    write_markdown_table(output_dir / "k_visible_multiseed_summary.md", summary, list(summary[0]) if summary else [])

    figure_macro_auprc(
        all_rows,
        path_prefix=output_dir / "figure_k_visible_macro_auprc_curve",
        pattern_order=list(reversed(selected_patterns)),
        method_order=list(args.methods),
        x_labels=["1", "2", "3", "4", "6", "8", "12"],
        xlabel="Visible lead count k",
        title="k-visible-lead degradation curve",
    )

    bootstrap_rows = []
    if args.bootstrap:
        run_by_key = {(run.method_id, run.seed): run for run in runs}
        low_k_patterns = ["k6_visible_random", "k4_visible_random", "k3_visible_random", "k2_visible_random", "k1_visible_random"]
        for run in runs:
        ...
        if bootstrap_rows:
            ...
            write_csv(output_dir / "k_visible_bootstrap_delta_ci.csv", bootstrap_rows, boot_columns)
            write_markdown_table(output_dir / "k_visible_bootstrap_delta_ci.md", bootstrap_rows, boot_columns)
            write_json(
                output_dir / "k_visible_bootstrap_run_config.json",
                {
                    "n_bootstrap": 20 if args.smoke_test else args.n_bootstrap,
                    "bootstrap_seed": args.bootstrap_seed,
    ...
    report_lines = [
        ...
        "- Pattern subsets are deterministic and paired across methods by sample index.",
        "- Outputs are supplementary and do not overwrite locked results.",
    ]
    markdown_report(output_dir / "k_visible_curve_report.md", "k-visible-lead Curve Evaluation", report_lines)
