#!/usr/bin/env python3
"""Evaluate A4b hierarchy ablation under missing-lead patterns."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate hierarchy ablation missing-pattern robustness.")
    ...
    parser.add_argument("--method-id", default="A4b_hierarchy_ablation")
    args = parser.parse_args()

    config = yaml.safe_load(args.config.read_text(encoding="utf-8"))
    if args.output_dir is not None:
        config.setdefault("paths", {})["output_dir"] = str(args.output_dir)
    config_used = Path(config.get("paths", {}).get("output_dir", "")) / "config_used.yaml"
    ...
    use_prediction_evaluator = args.save_predictions or args.splits != ["test"] or selected_patterns is not None
    results = {}
    if use_prediction_evaluator:
        ...
        for fill_mode in args.fill_modes:
            results[fill_mode] = evaluate_hierarchy_missing_patterns(
                checkpoint_path=args.checkpoint,
                config=config,
                fill_mode=fill_mode,
                smoke_test=args.smoke_test,
